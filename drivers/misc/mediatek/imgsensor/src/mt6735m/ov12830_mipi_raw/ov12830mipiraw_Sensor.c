...
/*lenovo-sw sunliang add for otp mid 2013_10_18 beging*/
static int OTP_SENSOR_MID=0;
static int OTP_SENSOR_LENSID=0;
/*lenovo-sw sunliang add for otp mid 2013_10_18 end*/
...
int check_otp_mid_lensid(int index)                                 
{                                                           
	int flag, i;                                     
	int bank, address;                               
	// select bank index                             
...
// return:              0,                                                 
int read_otp_mid_lensid(int index)                     
{                                                                          
	int i, bank;                                                    
	int address;                                                    
	int temp;                                                       
	...
	Sleep(5);                                                                      
	address = 0x3d00;                                                              
	OTP_SENSOR_MID=OV12830_read_cmos_sensor(address + 1);               
	OTP_SENSOR_LENSID=OV12830_read_cmos_sensor(address + 2);                            
	// clear otp buffer                                                            
	for (i=0;i<16;i++) {                                                           
		OV12830_write_cmos_sensor(0x3d00 + i, 0x00);                                
	}                                                                              
	OV12830DB("%s MID=0x%02x LENSID=0x%02x\r\n",__func__,OTP_SENSOR_MID,OTP_SENSOR_LENSID);
	return 0;                                                                      
}    
...
{                                                                          
	...
	OV12830DB("%s MID=0x%02x LENSID=0x%02x\r\n",__func__,(*otp_ptr).module_integrator_id,(*otp_ptr).lens_id);
	OTP_SENSOR_MID=(*otp_ptr).module_integrator_id;
	OTP_SENSOR_LENSID=(*otp_ptr).lens_id;
	// clear otp buffer                                                            
	for (i=0;i<16;i++) {                                                           
		OV12830_write_cmos_sensor(0x3d00 + i, 0x00);                                
	}                                                                              
...
{                                                                                                                           
	...
	if (i>3) {                                                                  
		...
		OV12830DB("%s no MID and LENSID data\n",__func__);
		return 1;                                                        
	}                                                                           
...
UINT32 OV12830Open(void)
{
	...
	for(i=0;i<3;i++)
	{
		...
		OV12830DB("OV12830 READ ID :%x",sensor_id);
...
UINT32 OV12830GetSensorID(UINT32 *sensorID)
{
	...
	OV12830_write_cmos_sensor(0x0103,0x01);// Reset sensor
    mDELAY(10);
	OV12830_write_cmos_sensor(0x0100,0x01);// strame on
	update_otp_mid_lensid();
	OV12830_write_cmos_sensor(0x0100,0x00);// strame off
    ...
    OV12830DB("%s SensorID=0x%04x MID=0x%02x LENSID=0x%02x\r\n",__func__,*sensorID,OTP_SENSOR_MID,OTP_SENSOR_LENSID);
    if ((*sensorID != OV12830_SENSOR_ID)|| (OTP_SENSOR_MID==0x15)){
	OV12830DB("SensorID ORMID Check Error\r\n");
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }
    return ERROR_NONE;
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    OV12830DB("OV12830FeatureControl: FeatureId = %d\n",FeatureId);
